...
This repository is updated with stable releases and matched with OIDC plugin. To follow active development, see: https://github.com/GopalSharma/moodle-auth_azureb2cfix

## Installation

1. Unpack the plugin into /auth/azureb2cfix within your Moodle install.
...
For more documentation, visit https://docs.moodle.org/37en/azureb2cfix

For more information including support and instructions on how to contribute, please see: https://github.com/GopalSharma/moodle-auth_azureb2cfix/README.md
...
<?php
	...
	$editcheck = get_user_preferences('auth_azureb2cfix_edit');
	if($editcheck == 0 || $editcheck == 2 ) {
		if($editcheck == 0) {
			set_user_preference('auth_azureb2cfix_edit', 2);
			header("Refresh:0");
		}
		if($editcheck == 2) {
			$userid = optional_param('id', null, PARAM_INT);
			if(!empty($idtoken[1]) && ($idtoken[0] =="#id_token")) {
				updateuserazureb2cfix($userid, $idtoken[1]);
				$_COOKIE['id_token'] = null;
				set_user_preference('auth_azureb2cfix_edit', 1);
			}
		}
	}
?>
...
```sh
function updateuserazureb2cfix($userid, $id_token) {
    global $CFG, $OUTPUT, $USER, $PAGE, $DB;
    require_once("{$CFG->dirroot}/auth/azureb2cfix/classes/loginflow/base.php");
    $idtoken = \auth_azureb2cfix\jwt::instance_from_encoded($id_token);
...
```
## Issues
Please post issues for this plugin to: https://github.com/GopalSharma/moodle-auth_azureb2cfix/issues
