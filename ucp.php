...
$azureb2cfixtoken = $DB->get_record('auth_azureb2c_token', ['userid' => $USER->id]);
$azureb2cfixconnected = (!empty($azureb2ctoken)) ? true : false;

$azureb2cfixloginconnected = ($USER->auth === 'azureb2c') ? true : false;
...
if (!empty($action)) {
    if ($action === 'connectlogin' && $azureb2cfixloginconnected === false) {
        ...
        if (!is_enabled_auth('azureb2cfix')) {
            throw new \moodle_exception('errorazureb2cfixnotenabled', 'auth_azureb2c');
        }
